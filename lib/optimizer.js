...
export function runOptimization({
  ...
  const testSet = {
    ...
    opens: slice(opens, splitIdx, undefined),
  ...
  const shared = { macro, fearGreedHistory, fundingRates, stopLossPct, takeProfitPct, allowShort, leverage, costPct, useBollinger, useStochRsi, useObv, useStrongCandle, useMarubozu, useSuperTrend, useDonchian, useSma, useMacd, useRsi, useFg, useMacro, useVolume, useNasdaq, useSp500, macroWeight, signalThreshold };
...
//
// coinDatasets: [{ coinId, symbol, prices, opens, highs, lows, volumes, dates, fundingRates }]
export function runMultiCoinOptimization({
  ...
  const shared = { macro, fearGreedHistory, stopLossPct, takeProfitPct, allowShort, leverage, costPct, useBollinger, useStochRsi, useObv, useStrongCandle, useMarubozu, useSuperTrend, useDonchian, useSma, useMacd, useRsi, useFg, useMacro, useVolume, useNasdaq, useSp500, macroWeight, signalThreshold };
  ...
  const perCoinSplit = coinDatasets.map((c) => {
    ...
    return {
      ...
      train: {
        ...
        opens: slice(c.opens, 0, splitIdx),
      ...
      test: {
        ...
        opens: slice(c.opens, splitIdx, undefined),
...
function sliceSet(prices, opens, highs, lows, volumes, dates, from, to) {
  return {
    prices: prices.slice(from, to),
    opens: slice(opens, from, to),
...
export function runWalkForward({
  ...
  for (let fold = 1; fold <= folds; fold++) {
    ...
    const trainSet = sliceSet(prices, opens, highs, lows, volumes, dates, 0, trainEnd);
    const testSet = sliceSet(prices, opens, highs, lows, volumes, dates, trainEnd, testEnd);
...
function sliceCoin(c, from, to) {
  return {
    ...
    symbol: c.symbol,
    fundingRates: c.fundingRates,
    ...sliceSet(c.prices, c.opens, c.highs, c.lows, c.volumes, c.dates, from, to),
  };
}
