...
// DOGE am 16.08.2026 ergänzt (auf Nachfrage von [[user-nico-cotterill]], siehe
// README "Coin-Auswahl") -- bewusst statt eines neueren Listings gewählt:
// DOGEUSDT ist seit 2019 auf Binance gelistet, verkürzt also NICHT das
// gemeinsame Multi-Coin-Walk-Forward-Zeitfenster (aktuell durch TAOs ~850
// Tage begrenzt) und bringt zugleich deutlich andere, sentimentgetriebenere
// Kursdynamik als die übrigen fünf Coins ein.
export const COINS = [
  ...
  const results = await Promise.all(
    COINS.map(async (coin) => {
      ...
      return {
        ...
        // Für Preis-Alarme (pages/api/cron/check-alerts.js): steckt im selben
        // Ticker-Aufruf bereits mit drin, kein Zusatz-Request nötig.
...
// YoY/Trend-Fenster). DTWEXBGS/DGS10/VIXCLS/SP500/NASDAQCOM sind täglich
// (Handelstage, keine Wochenenden/Feiertage) – dailyLimit=130 deckt fürs
// Dashboard komfortabel das längste Trendfenster ab (Nasdaq nutzt 90
// Handelstage, siehe computeMacroRegime -> etwas Puffer für Feiertage).
// Der Backtest übergibt für beide deutlich größere Limits, damit an jedem
// Simulationstag genug Vorlauf vorhanden ist.
export async function fetchMacroData(monthlyLimit = 24, dailyLimit = 130) {
  ...
  const [m2, fedfunds, dxy, yield10y, vix, sp500, nasdaq] = await Promise.all([
    ...
    fetchSeries("SP500", dailyLimit), // S&P 500 Index (täglich)
    fetchSeries("NASDAQCOM", dailyLimit), // Nasdaq Composite Index (täglich)
  ]);
  return { m2, fedfunds, dxy, yield10y, vix, sp500, nasdaq };
}
...
export async function fetchHistoricalSeries(coinId, days = 365) {
  ...
  return {
    ...
    opens: klines.map((k) => ({ t: k[0], v: parseFloat(k[1]) })),
...
// Orderbuch-Snapshot (Spot) pro Coin -- kostenlos, kein Key nötig. Liefert nur
// den aktuellen Zustand (keine Historie über die REST-API verfügbar), daher
// wie fetchWhaleData nur fürs Live-Dashboard nutzbar, nicht für den Backtest.
// Zwei Kennzahlen: Geld-Brief-Spanne (wie teuer ist ein sofortiger Handel
// gerade) und kumulierte Tiefe innerhalb ±1% vom Mittelkurs (wie viel lässt
// sich handeln, ohne den Kurs nennenswert zu bewegen) -- reine Markt-
// Kontextinfo, kein Kaufen/Verkaufen-Signal.
export async function fetchOrderBookData() {
  const results = await Promise.all(
    COINS.map(async (coin) => {
      const r = await fetch(`https://api.binance.com/api/v3/depth?symbol=${coin.pair}&limit=100`);
      if (!r.ok) throw new Error(`Binance Orderbuch-Fehler (${r.status}) für ${coin.name}`);
      const data = await r.json();
      ...
      // Bids sind absteigend, Asks aufsteigend sortiert -- Summe stoppen,
      // sobald ein Preis außerhalb der ±1%-Grenze liegt.
      const depthWithin = (levels, lowerBound, upperBound) => {
        let sum = 0;
        for (const [priceStr, qtyStr] of levels) {
          const price = parseFloat(priceStr);
          if (price < lowerBound || price > upperBound) break;
          sum += price * parseFloat(qtyStr);
        }
        return sum;
      };
      const bidDepthUsd = depthWithin(data.bids, mid * 0.99, mid);
      const askDepthUsd = depthWithin(data.asks, mid, mid * 1.01);

      return [coin.id, { spreadPct, depthUsd: bidDepthUsd + askDepthUsd }];
    })
  );
  return Object.fromEntries(results);
}
