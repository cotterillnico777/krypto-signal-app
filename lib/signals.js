...
export function explainSignal({ label, smaSig, rsi, macdSig, volSig, macro, fg, whaleSig, rsiBuyThreshold = 30, rsiSellThreshold = 70, macroWeight = 2.0 }) {
  const bullish = label === "Kaufen" || label === "Halten (bullish)";
  const bearish = label === "Verkaufen" || label === "Halten (bearish)";
  if (!bullish && !bearish) {
    return "Die Signale sind aktuell gemischt bzw. neutral -- keine klare Tendenz erkennbar.";
  }
  ...
  if (smaSig && smaSig.dir !== 0) {
    factors.push({
      ...
      text: smaSig.dir > 0 ? "der kurzfristige Kurstrend über dem langfristigen liegt (positiv)" : "der kurzfristige Kurstrend unter dem langfristigen liegt (negativ)",
    });
  }
  if (macdSig && macdSig.dir !== 0) {
    factors.push({
      ...
      text: macdSig.dir > 0 ? "das MACD-Momentum positiv ist" : "das MACD-Momentum negativ ist",
    });
  }
  if (rsi != null) {
    if (rsi <= rsiBuyThreshold) factors.push({ weight: 0.8, dir: 1, text: "der RSI überverkauft ist (mögliche Erholung)" });
    if (rsi >= rsiSellThreshold) factors.push({ weight: 0.8, dir: -1, text: "der RSI überkauft ist (mögliche Korrektur)" });
  }
  if (fg != null) {
    if (fg <= 25) factors.push({ weight: 0.5, dir: 1, text: "der Fear-&-Greed-Index extreme Angst zeigt (historisch oft ein Kaufsignal)" });
    if (fg >= 75) factors.push({ weight: 0.5, dir: -1, text: "der Fear-&-Greed-Index extreme Gier zeigt (historisch oft ein Warnsignal)" });
  }
  if (macro) {
    if (macro.label === "Risk-on") factors.push({ weight: macroWeight, dir: 1, text: "das gesamtwirtschaftliche Umfeld günstig ist (Risk-on)" });
    if (macro.label === "Risk-off") factors.push({ weight: macroWeight, dir: -1, text: "das gesamtwirtschaftliche Umfeld ungünstig ist (Risk-off)" });
  }
  if (volSig && volSig.dir > 0) {
    factors.push({ weight: volSig.dir * 0.4, dir: 1, text: "das Handelsvolumen deutlich erhöht ist" });
  }
  if (whaleSig && whaleSig.dir !== 0) {
    factors.push({
      weight: 0.4,
      dir: whaleSig.dir,
      text: whaleSig.dir > 0 ? "große Trader stärker long positioniert sind als sonst" : "große Trader stärker short positioniert sind als sonst",
    });
  }

  const aligned = factors.filter((f) => f.dir === wantDir).sort((a, b) => b.weight - a.weight);
  if (aligned.length === 0) {
    return "Die Einzelfaktoren stehen knapp gegeneinander -- keine dominante Begründung erkennbar.";
  }
  const top = aligned.slice(0, 2).map((f) => f.text);
  const joined = top.length === 2 ? `${top[0]} und ${top[1]}` : top[0];
  return `Vor allem, weil ${joined}.`;
}
