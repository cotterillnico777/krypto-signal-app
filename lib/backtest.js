...
//
// takeProfitPct: z.B. 20 => Position wird geschlossen, sobald das Tageshoch/-tief
// 20% zugunsten des Einstiegspreises läuft (Ausführung zum Zielpreis, gleiche
// Vereinfachung wie beim Stop-Loss). null/0 = aus (Standard) – bisher hatte die
// Strategie kein Gewinnziel, Positionen liefen bis Stop/Gegensignal/Datenende.
// An Tagen, an denen Stop UND Target rechnerisch in derselben Tagesspanne
// lägen, hat der Stop-Loss Vorrang (siehe Prüfreihenfolge unten) – konservative
// Annahme, da nur Tages-OHLC vorliegt, keine echte Intraday-Reihenfolge.
//
// Empirisch geprüft (11.08.2026, Multi-Coin Walk-Forward über 365/730/850
// Tage, sechs Kombinationen: 15/20/30% Take-Profit solo sowie 20/30% im
// Bracket mit 15% Stop-Loss): bleibt standardmäßig AUS – und zwar nicht nur
// unbewiesen, sondern in den beiden längeren/verlässlicheren Fenstern
// durchgängig SCHLECHTER als die Baseline ohne Take-Profit (730 Tage:
// profitableFolds fällt bei jeder Variante auf 0 von 4, Baseline 1 von 4;
// 850 Tage: bis zu -11% Rendite vs. Baseline +13,8%). Nur im rauschigsten
// 365-Tage-Fenster (4 Folds) sah es kurz besser aus. Plausible Erklärung:
// die Strategie schließt ohnehin signalgetrieben (SMA/MACD-Crossover) und
// lässt Gewinner laufen – ein festes Gewinnziel kappt genau diese
// ausgedehnten Krypto-Trends vorzeitig. Bewusst als API-Parameter erhalten
// (kein Blocker für weitere Experimente mit anderen Prozentsätzen), aber
// nicht im Dashboard/`pages/backtest.js` als UI-Option angeboten, um ein
// nachweislich eher schädliches Setting nicht prominent zu bewerben.
//
...
export function runBacktest({
  ...
  const marubozuSeries = opens && highs && lows ? computeMarubozuSeries(opens, highs, lows, prices) : null;
  const superTrendTrend = highs && lows ? computeSuperTrendSeries(highs, lows, prices) : null;
  const { upper: donchianUpper, lower: donchianLower } = highs && lows ? computeDonchianSeries(highs, lows) : { upper: null, lower: null };
  ...
  for (let i = 0; i < prices.length; i++) {
    ...
    // Take-Profit: nur geprüft, wenn Stop-Loss/Liquidation oben nicht bereits
    // ausgelöst hat (direction wäre sonst schon null) -- an Tagen, an denen
    // rechnerisch beide Preise innerhalb der Tagesspanne liegen, hat der
    // Stop-Loss Vorrang. Gleiche vereinfachende Annahme wie beim Stop-Loss
    // selbst: Ausführung zum Zielpreis, kein zeitlich aufgelöster Intraday-
    // Verlauf verfügbar (nur Tages-OHLC).
    if (direction && takeProfitPct) {
      if (direction === "long") {
        const targetPrice = entryPrice * (1 + takeProfitPct / 100);
        if (high >= targetPrice) {
          closePosition(targetPrice, dateStr, { tookProfit: true });
        }
      } else if (direction === "short") {
        const targetPrice = entryPrice * (1 - takeProfitPct / 100);
        if (low <= targetPrice) {
    ...
    if (i >= WARMUP_DAYS) {
      ...
      const marubozuSig = marubozuSeries ? marubozuSeries[i] ?? { label: "n/a", dir: 0 } : { label: "n/a", dir: 0 };
      const superTrendSig = superTrendTrend ? superTrendSignalAt(superTrendTrend, i) : { label: "n/a", dir: 0 };
      const donchianSig = donchianUpper ? donchianSignalAt(prices, donchianUpper, donchianLower, i) : { label: "n/a", dir: 0 };
      ...
      sp500Cursor = advanceCursor(macro.sp500 || [], sp500Cursor, dateStr);
      nasdaqCursor = advanceCursor(macro.nasdaq || [], nasdaqCursor, dateStr);

      const macroAsOf = computeMacroRegime(macro.m2, macro.fedfunds, macro.dxy, macro.yield10y, macro.vix, macro.sp500 || [], macro.nasdaq || [], {
        m2: m2Cursor,
      ...
      combined = combineSignal(smaSig, rsi, macroAsOf, fg, macdSig, volSig, {
        ...
        marubozuSig: useMarubozu ? marubozuSig : null,
        superTrendSig: useSuperTrend ? superTrendSig : null,
        donchianSig: useDonchian ? donchianSig : null,
        useSma,
        useMacd,
        useRsi,
        useFg,
