...
Abgedeckt: die 6 öffentlichen Seiten (`/login`, `/signup`, `/track-record`, `/validation`, `/glossar`, `/upgrade`), das Dashboard (`pages/index.js`) inkl. `AppHeader`-Navigation, `TrialBanner`, `OnboardingTour` und aller Signal-Erklärungen, die vier Analyse-Tools `/backtest`, `/optimize`, `/walkforward`, `/portfolio` (gemeinsame Bausteine unter `tools.shared.*` in `lib/i18n.js`) sowie die restlichen Journal-/Tool-Seiten `/holdings`, `/trades`, `/alerts`, `/risk-reward`, `/chart-analysis` und die kleinen Komponenten `PushSubscribeButton`/`InstallPrompt`. Damit ist die komplette App zweisprachig -- jede Seite hinter dem Login nutzt eigene Übersetzungs-Namensräume in `lib/i18n.js` (`holdings.*`, `trades.*`, `alerts.*`, `riskReward.*`, `chartAnalysis.*`, `pushButton.*`, `installPrompt.*`), analog zum Muster der öffentlichen Seiten.
