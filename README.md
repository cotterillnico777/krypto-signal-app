...
Abgedeckt sind aktuell: die 6 öffentlichen Seiten (`/login`, `/signup`, `/track-record`, `/validation`, `/glossar`, `/upgrade`), das Dashboard (`pages/index.js`) inkl. `AppHeader`-Navigation, `TrialBanner`, `OnboardingTour` und aller Signal-Erklärungen, sowie – als dritter Ausbauschritt – die vier Analyse-Tools `/backtest`, `/optimize`, `/walkforward` und `/portfolio` (gemeinsame Bausteine wie PERIODS/COSTS/STOP_LOSSES-Regler, Ergebnis-Kennzahlen und Tabellen-Header liegen unter `tools.shared.*` in `lib/i18n.js`, seitenspezifische Texte unter `tools.backtest.*`/`tools.optimize.*`/etc.). Die übrigen gegateten Journal-/Tool-Seiten (Mein Portfolio, Trades, R:R-Rechner, Preis-Alarme, Chart-Analyse) sind noch nicht übersetzt und bleiben Deutsch, obwohl der Umschalter dort (über den gemeinsamen `AppHeader`) schon sichtbar ist – geplanter nächster Schritt.
